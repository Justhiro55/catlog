...
async fn main() -> Result<()> {
    ...
    if let Some(file_path) = args.follow {
    ...
    let stdout_handle = tokio::task::spawn_blocking(move || {
        for line in stdout_reader.lines() {
            if let Ok(line) = line {
                ...
                rt.block_on(async {
                    let _ = process_line(&line, &args_clone, status_filter).await;
                });
            }
        }
    });

    let args_clone2 = Args {
        follow: args.follow.clone(),
        exec: args.exec.clone(),
        size: args.size,
        ascii: args.ascii,
    ...
    let stderr_handle = tokio::task::spawn_blocking(move || {
        for line in stderr_reader.lines() {
            if let Ok(line) = line {
                let rt = tokio::runtime::Runtime::new().unwrap();
                rt.block_on(async {
                    let _ = process_line(&line, &args_clone2, status_filter).await;
                });
            }
        }
    });
...
async fn display_cat_for_status(code: u16, args: &Args) -> Result<()> {
    ...
    if !args.no_image {
        if let Ok(image_url) = cat_api::fetch_cat_image().await {
            if let Ok(image_data) = cat_api::download_image(&image_url).await {
